...
#define vec_random_idx(v)\
  generate_random_int(0, (v)->length - 1)


#define vec_random(v)\
  (v)->data[vec_random_idx(v)]


#define vec_valid_idx(v, idx)\
  ((idx) >= 0 && (idx) < (v)->length)
...
#define vec_foreach(v, var, iter)\
  for ((iter) = 0;\
       (iter) < (v)->length && ((v)->length > 0 ? ((var) = (v)->data[(iter)], 1) : 0);\
       ++(iter))


#define vec_foreach_rev(v, var, iter)\
  for ((iter) = (v)->length - 1;\
       (iter) >= 0 && ((v)->length > 0 ? ((var) = (v)->data[(iter)], 1) : 0);\
       --(iter))
