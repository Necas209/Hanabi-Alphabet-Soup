...
card_t load_card(const cJSON *const card_json) {
    ...
    cJSON *number_known = cJSON_GetObjectItem(card_json, "number_known");
    cJSON *color_known = cJSON_GetObjectItem(card_json, "color_known");
    // Create a card object
    card_t card = {
            .number = cJSON_GetObjectItem(card_json, "number")->valueint,
            .color = cJSON_GetObjectItem(card_json, "color")->valueint,
            .number_known = cJSON_IsTrue(number_known),
            .color_known = cJSON_IsTrue(color_known)
    };
    return card;
}